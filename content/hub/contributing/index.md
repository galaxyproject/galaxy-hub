...
## News and events metadata: subsites and main_subsite

News (`content/news/`) and events (`content/events/`) items have two frontmatter fields that are often copied blindly from older items: `subsites` and `main_subsite`. What they do — which subsite listings include an item, and how they drive social-media posting via feed consumers like the [galaxy-social-assistant](https://github.com/usegalaxy-eu/galaxy-social-assistant) — is explained centrally in the repo's [CONTRIBUTING.md → Subsites and `main_subsite` in news and events](https://github.com/galaxyproject/galaxy-hub/blob/main/CONTRIBUTING.md#subsites-and-main_subsite-in-news-and-events).

If your pull request on github touches news or events items you will be pointed to the same explanation by an autogenerated comment.
