...
I used the [Concatenate datasets tail-to-head (cat)](https://usegalaxy.eu/?tool_id=tp_cat) tool to merge them. I simply dragged and dropped my FASTQ files, crossed my fingers, and clicked run. To be honest, it was pretty tedious. I had 188 FASTQ files to run through it, manually making sure I was selecting the correct replicates and conditions.
...
Desperate, I reached out to the Galaxy experts. I asked [Pavan](https://galaxyproject.org/hall-of-fame/pavanvidem/) to help me play detective. He kindly agreed, and we started digging. Why was the alignment rate so low? Why did some samples have >90% alignment while others struggled to hit 1%? Did the [fastp](https://usegalaxy.eu/?tool_id=fastp) tool somehow aggressively trim away all my reads? Was there a bug in the forward/reverse pair assignment during alignment? Did the facility send me garbage data?
...
Tada! **95.5% mapped for each mate-pair!** How could this be? Did the facility mess up the forward and reverse assignment? That would be incredibly rare and we can clearly see the mate pair ids in read names. For eg, All read IDs in R1 contain `1:N:0:TACGC..` R2 contain `2:N:0:TACGA...`. These are autogenerated from the sequencing machine. So the file names and contents are correct.
...
We pinged [Wolfgang](https://galaxyproject.org/hall-of-fame/wm75/) for help creating a nested collection (a list within a list). As it turns out, the task is actually quite easy... well, if you know how the collection tools work! :P
