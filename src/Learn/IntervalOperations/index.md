...
When complementing a chromosome, the length of the chromosome is needed.  Galaxy uses the `chromInfo` tables available through the <<nwwl(UCSC)>> Table Browser for this information.  For complements on builds not available through <<nwwl(UCSC)>>, a default chromosome length of 512 megabases is assumed.
...
* `Return only records that are joined` will only return intervals in the first query that overlap and are joined to an interval in the second query.  For users of SQL databases, this is similar to an “<<nwwl(INNER)>> <<nwwl(JOIN)>>”.
