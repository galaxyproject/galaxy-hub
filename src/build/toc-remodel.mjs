 ...
 * of the body contents. This will create two top-level wrapper elements with the table of contents in the first one and
 * the body in the second.
 * @param {Object} [opts] Options.
 * @param {Object} [opts.tocAttrs] Values for any HTML attributes to insert into the tag wrapping the table of contents.
 *   WARNING: The unsanitized values are inserted literally between double quotes into the string, so mind the content.
 * @param {Object} [opts.bodyAttrs]  Same as `opts.tocAttrs` but for the body wrapper.
 * @param {String} [opts.wrapperTag="section"] The name of the tag to use to wrap the ToC and body.
 *   WARNING: This value is inserted into the HTML unsanitized, so ensuring it's a valid tag is your responsibility.
 */
export default function attacher(opts) {
    if (opts === undefined) {
        opts = {};
    }
    let wrapper = opts.wrapperTag || "section";
    function transformer(tree) {
        if (hasAutoToc(tree)) {
            removeNode(tree, "heading", HEADING_TEXT);
            removeNode(tree, "heading", HEADING_ENDING);
            // Wrap the ToC.
            wrapNodes(tree, TOC_START, TOC_SIZE - 1, wrapper, opts.tocAttrs);
            // Wrap the body.
            wrapNodes(tree, TOC_START + TOC_SIZE + 1, tree.children.length, wrapper, opts.bodyAttrs);
...
function wrapNodes(tree, start, end, wrapperTag, wrapperAttrs = {}) {
    let wrapperStart = {
        type: "html",
        value: makeTag(wrapperTag, wrapperAttrs),
    };
    let wrapperEnd = {
        type: "html",
        value: `</${wrapperTag}>`,
