...
1. Update code to the chosen release branch (release_18.01 here).
 * New Galaxy repository: `$ git clone -b release_18.01 https://github.com/galaxyproject/galaxy.git`
 * Update of existing repository: `$ git checkout release_18.01 && git pull --ff-only origin release_18.01`
...
1. Diff `config/galaxy.yml` and `config/galaxy.yml.sample` to check for possible new features/changes,
...
1. Run ``make config-lint`` and ``make config-validate`` to check your configuration against the new version of Galaxy.
