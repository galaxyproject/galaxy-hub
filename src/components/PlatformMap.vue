        ...
        <div v-if="platformsWithLocations.length > 0" class="map-wrapper">
            ...
            <div class="map-legend">
                ...
                <p class="text-muted mb-0">
                    <small>{{ platformsWithLocations.length }} platform{{ platformsWithLocations.length !== 1 ? 's' : '' }} with location data</small>
                </p>
            </div>
        </div>
...
<script>
// OpenLayers imports - will be transpiled by babel
import 'ol/ol.css';
    ...
    methods: {
            ...
            return new Promise((resolve) => {
                if (platform.image) {
                    ...
                    if (iconUrl && iconUrl.trim() !== '') {
                        ...
                        this.createCompositeMarker(iconUrl).then(compositeUrl => {
        ...
        createCompositeMarker(logoUrl) {
            return new Promise((resolve, reject) => {
                ...
                // Create the marker background SVG
                const markerSvg = `
        ...
        showPopup(coordinate, platform) {
            ...
            popupElement.innerHTML = `
                <div class="popup-header">
                    ${platform.image ? `<img src="${this.getImageUrl(platform.image)}" alt="${platform.title}" class="popup-logo" />` : ''}
...
.galaxy-dark-mode {
    filter: 
        invert(90%)           /* Invert colors */
        hue-rotate(180deg)    /* Adjust hue to match Galaxy theme */
        brightness(0.9)       /* Slightly dim */
...
.galaxy-pale-mode {
    filter: 
        sepia(0%)            /* Add slight warm tone */
        saturate(0.6)         /* Desaturate for pale look */
        hue-rotate(20deg)    /* Shift to blue-gray */
        brightness(1.15)      /* Lighten overall */
        contrast(1);       /* Reduce contrast for softer appearance */
}
