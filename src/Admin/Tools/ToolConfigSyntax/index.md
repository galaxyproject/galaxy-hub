...
The command tag should be started and finished by a [CDATA](https://en.wikipedia.org/wiki/CDATA) tag, allowing direct use of characters like the ampersand (“&”) without needing XML escaping (“&amp;amp;”).
```
#!highlight xml
...
<table>
  ...
  <tr>
    <td> argument </td>
    <td> a string </td>
    <td> If the parameter reflects just one command line argument of a certain tool, this tag should be set to that particular argument. It is rendered in parenthesis after the help section. </td>
    <td> no </td>
    <td> <code>argument="--sensitive"</code> </td>
  </tr>
</table>
...
<table>
  ...
  <tr>
    <td> multiple </td>
    <td> true, false </td>
    <td> Only if "type" attribute value is "data". <a href='https://github.com/galaxyproject/galaxy/blob/dev/test/functional/tools/multi_data_param.xml '>example tool</a> </td>
    <td> no </td>
...
<table>
  ...
  <tr>
    ...
    <td> * Only if "type" attribute value is "select" - used with select lists whose options are dynamically generated from the results returned by a function contained within a code file. The output should be a list, where each item matches the format: (name,value,BOOL) </td>
    <td> no </td>
...
The following is taken from the [~/tools/plotting/xy_plot.xml](http://github.com/galaxyproject/tools-devteam/blob/master/tools/xy_plot/xy_plot.xml) tool config.
...
<table>
  ...
  <tr>
    <td> label </td>
    <td> a string </td>
    <td> This will be the label of the history item for the output data set.  The string can include structure like <code>${<some param name>.<some attribute>} </code>, as discussed for command line parameters in the <code><command></code> tag set section above. The default label is <code>"${tool.name} on ${on_string}"</code> </td>
    <td> no </td>
  ...
  <tr>
    <td> auto_format </td>
    <td> True, true, False, false </td>
    <td> Whether or not to let Galaxy auto-detect the file format. </td>
    <td> no </td>
    <td> <code>auto_format="true"</code> </td>
  </tr>
</table>
...
The help tag should be started and finished by a [CDATA](https://en.wikipedia.org/wiki/CDATA) tag.
```
#!highlight xml
...
### Parameterizing XML Macros, 2

In addition to using yield blocks, there is another way to parameterize macros through the use of specifying ```token_xyz``` attributes on the macro definition, and then using ```@XYZ@``` anywhere within the XML.
...
* [Galaxy Intergalactic Utilities Commission Standards and Best Practices](https://galaxy-iuc-standards.readthedocs.org/)
* [Building Galaxy Tools with Planemo tutorial](https://planemo.readthedocs.org/en/latest/writing.html)
