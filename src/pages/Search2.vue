      ...
      <div class="filters">
        ...
        <div class="filter">
          ...
          <div class="options-list">
            <label
              v-for="tag in filteredUniqueTags"
              :key="tag"
              class="option"
            >
              <input
        ...
        <div class="filter">
          ...
          <div class="options-list">
            <label
              v-for="site in filteredUniqueSubsites"
              :key="site"
              class="option"
            >
              <input
        ...
        <div class="filter">
          ...
          <div class="options-list">
            <label
              v-for="auth in filteredUniqueAuthors"
              :key="auth"
              class="option"
            >
              <input
        ...
        <div class="filter">
          ...
          <div class="options-list">
            <label
              v-for="sup in filteredUniqueSupporters"
              :key="sup"
              class="option"
            >
              <input
      ...
      <div class="results" v-if="filteredPosts.length">
        ...
        >
          ...
          <p class="meta">
            ...
            <span v-if="post.tags">Tags: {{ post.tags.join(', ') }}</span><br />
            <span v-if="post.subsites">Subsites: {{ post.subsites.join(', ') }}</span><br />
...
export default {
  ...
  computed: {
    ...
    uniqueAuthors() {
      return [...new Set(
        this.posts.flatMap(p => [p.authors, p.contact].filter(Boolean))
      )].sort();
    },
    uniqueSupporters() {
    ...
    filteredUniqueTags() {
      return this.uniqueTags.filter(t =>
        t.toLowerCase().includes(this.tagSearch.toLowerCase())
      );
    },
    filteredUniqueSubsites() {
      return this.uniqueSubsites.filter(s =>
        s.toLowerCase().includes(this.subsiteSearch.toLowerCase())
      );
    },
    filteredUniqueAuthors() {
      return this.uniqueAuthors.filter(a =>
        a.toLowerCase().includes(this.authorSearch.toLowerCase())
      );
    },
    filteredUniqueSupporters() {
      return this.uniqueSupporters.filter(s =>
        s.toLowerCase().includes(this.supporterSearch.toLowerCase())
      );
    },
    filteredPosts() {
      return this.posts.filter(p => {
        ...
        if (
          this.titleSearch &&
          !p.title.toLowerCase().includes(this.titleSearch.toLowerCase())
        ) {
          return false;
        }
        // Date range filter
