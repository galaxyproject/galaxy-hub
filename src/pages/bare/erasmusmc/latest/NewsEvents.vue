...
query {
    news: allArticle(
        limit: 5, filter: {category: {eq: "news" }, subsites: {contains: ["erasmusmc"]}, draft: {ne: true}}
    ) {
        totalCount
        edges {
            node {
    ...
    events: allArticle(
        ...
        filter: {
            category: {eq: "events"}, subsites: {contains: ["erasmusmc"]}, has_date: {eq: true}, days_ago: {lte: 0},
            draft: {ne: true}
        }
    ) {
        totalCount
