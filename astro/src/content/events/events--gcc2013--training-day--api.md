---
slug: events/gcc2013/training-day/api
---
## Table of contents

* [The Galaxy API](#the-galaxy-api)
  * [What's an API and why would I care?](#whats-an-api-and-why-would-i-care)
* [How to Access and Use the API](#how-to-access-and-use-the-api)
  * [The concepts of resources, verbs, and the REST model](#the-concepts-of-resources-verbs-and-the-rest-model)
* [A Sample Use](#a-sample-use)
* [The Set Up](#the-set-up)
  * [The API Key](#the-api-key)
  * [Structure](#structure)
* [The Steps](#the-steps)
  * [1. Checking your key with the user API](#1-checking-your-key-with-the-user-api)
  * [2. Looking at your histories with the history API](#2-looking-at-your-histories-with-the-history-api)
    * [histories\_1.py](#histories_1py)
    * [histories\_2.py](#histories_2py)
* [More Resources and Thanks](#more-resources-and-thanks)
...
Taking a look inside users\_1.py we can see we're simply calling a function in our `main` clause: get\_users. It takes
...
In histories\_2.get\_history, we're building the url and including an ID in it to get data about a specific history.
...
as each step\_N will add new functionality to the overall, final script. (Warning: the effects of calling the resource
...
Since each step\_N.py script will build on the previous, we'll be creating a new history for each step and each new
...
hdas\_1.py will do an `index` of the hdas if you call it from the command line. Examine hdas\_1.get\_hdas. In some ways
it's very similar to histories.get\_history (the `show`) but we've added `/contents` to the URL path.
...
It's the initial data used in the screencast quickie: http://screencast.g2.bx.psu.edu/quickie\_13\_fastq\_basic/flow.html

The tools\_1.py contains one function: upload\_hda. The Galaxy developers would like to have every tool be available to
...
However, the same API method from `datasets_2` (\`GET + resource\_url + ID), when passed some optional parameters
...
(Also note the print out of the get\_dataset\_column full url: see `step_8.py` and `datasets_3.py` for an explanation of
