...
slug: events/gmod-summer-school2014
---
## Table of contents

* [Introduction](#introduction)
* [Command-line Set Up and Operation](#command-line-set-up-and-operation)
  * [A 'galaxy' user](#a-galaxy-user)
  * [Downloading and Installing Galaxy](#downloading-and-installing-galaxy)
  * [The universe\_wsgi.ini Configuration File](#the-universe_wsgiini-configuration-file)
  * [Installing PostgreSQL](#installing-postgresql)
  * [More Configuration](#more-configuration)
    * [Debugging](#debugging)
    * [Security](#security)
    * [Port Number and customization](#port-number-and-customization)
    * [The tool\_dependency\_dir](#the-tool_dependency_dir)
  * [run.sh: Start Up, Stopping, Re-starting and Log Files](#runsh-start-up-stopping-re-starting-and-log-files)
    * [In the foreground](#in-the-foreground)
    * [Restarting the server](#restarting-the-server)
    * [The log file](#the-log-file)
* [The Admin Web Interface](#the-admin-web-interface)
  * [Libraries](#libraries)
    * [Example data](#example-data)
    * [library\_import\_dir](#library_import_dir)
    * [A data library for raw isolate reads](#a-data-library-for-raw-isolate-reads)
  * [Tools](#tools)
    * [Tool Sheds](#tool-sheds)
    * [Installing a tool from a Tool Shed](#installing-a-tool-from-a-tool-shed)
    * [Exercise: building a QC pipeline](#exercise-building-a-qc-pipeline)
      * [1. Copy isolate data](#1-copy-isolate-data)
      * [(Aside: Histories)](#aside-histories)
      * [2. Join the paired-end read files](#2-join-the-paired-end-read-files)
      * [3. Filter the joined reads](#3-filter-the-joined-reads)
      * [4. Re-split the joined, filtered file](#4-re-split-the-joined-filtered-file)
      * [5. Re-split the joined, filtered file](#5-re-split-the-joined-filtered-file)
  * [Workflows](#workflows)
    * [Manually creating and editing workflows](#manually-creating-and-editing-workflows)
    * [Extracting a workflow from a history](#extracting-a-workflow-from-a-history)
    * [Publishing a workflow](#publishing-a-workflow)
    * [Workflow notes](#workflow-notes)
    * [Testing our QC workflow](#testing-our-qc-workflow)
* [More Resources](#more-resources)
  * [Galaxy](#galaxy)
  * [Unix System Administration](#unix-system-administration)
  * [Advanced Configuration for Production Scale Galaxy Servers](#advanced-configuration-for-production-scale-galaxy-servers)
...
### The universe\_wsgi.ini Configuration File
...
<http://www.ncbi.nlm.nih.gov/sra/SRX477904>
...
<https://usegalaxy.org/u/carleberhard/h/saccer-srr1182403>

#### library\_import\_dir
...
5. Change 'Upload option' to 'Upload a directory of files' using the drop down menu.
6. Change 'File Format' to `fastqsanger`. You can do this by clicking the drop down, typing `fastqsanger` to search
   for it or scrolling to it with your keys, and clicking that option. This is Galaxy's preferred flavor of fastq.
7. Change 'Server Directory' to our directory
8. Change 'Copy data into Galaxy?' to `link to files without copying into Galaxy` - this allows us to not duplicate
   large data files.
9. Leave the remaining options at their defaults
10. Click 'Upload to library'. A new page will load displaying the status of the upload which should complete quickly.
...
2. Click both 'SRR1182403\_1.fastq' and 'SRR1182403\_2.fastq' to select them
...
3. The center panel will open with a form that controls the parameters the tools will use. The joiner is relatively
   ...
   parameter settings.
4. Click the select dropdown under 'Left-hand reads' and then choose 'SRR1182403\_1.fastq'
5. Click the select dropdown under 'Right-hand reads' and then choose 'SRR1182403\_2.fastq'
...
4. 0 in the 'Minimum Quality' field. This is the minimum phred score a read base must have in order to be included in
   the output dataset.
5. Click 'Execute' and wait for the job to finish and the dataset's color to turn green.
...
2. Select 'SRR1182403\_1.fastq' and 'SRR1182403\_2.fastq' on the left hand side.
...
9. Select '1: SRR1182403\_1.fastq' for the first 'Input dataset' (Step 1 on the page)
10. Select '2: SRR1182403\_2.fastq' for the second 'Input dataset' (Step 2 on the page)
